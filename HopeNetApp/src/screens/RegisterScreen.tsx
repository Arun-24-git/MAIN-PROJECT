...
import {
  ...
  Dimensions, Keyboard, StatusBar, Modal, FlatList
} from 'react-native';
...
export default function RegisterScreen({ navigation }: any) {
  ...
  const handleRegister = async () => {
    ...
    if (phone.length !== 10) {
      Alert.alert("⚠️ VALIDATION ERROR", "Emergency node requires exactly 10 numeric digits.");
      return;
    }
    ...
    try {
      ...
      // --- FIXED LOGIC BELOW: Removed "..." and kept only 8 characters ---
      Alert.alert(
        "✅ IDENTITY SECURED",
        `Registered: ${fullNumber}\n\nID: ${id.substring(0, 8)}`,
        [{
          text: "OK",
          onPress: () => {
            setPhone('');
      ...
      Alert.alert("❌ FAILED", "Secure storage write error.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.mainWrapper}>
        <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>OFFLINE MESH NETWORK</Text>
            ...
            <View style={styles.cardPadding}>
              ...
              {loading ? (
                <View style={styles.loadingBox}>
                  ...
                  <View style={styles.barContainer}><Animated.View style={[styles.barFill, { width: progressAnim.interpolate({ inputRange: [0, 1], outputRange: ['0%', '100%'] }) }]} /></View>
                </View>
              ) : (
                <Animated.View style={{ transform: [{ scale: phone.length === 10 ? buttonPulseAnim : 1 }] }}>
                  ...
                  >
                    <Text style={[styles.btnText, phone.length < 10 && { color: 'rgba(0,0,0,0.3)' }]}>REGISTER</Text>
                  </TouchableOpacity>
                </Animated.View>
              )}
            </View>
          </View>
          <View style={styles.footer}><Text style={styles.footerText}>🛡️ SECURE • DECENTRALIZED • OFFLINE</Text></View>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  backgroundVideo: { position: 'absolute', top: 0, left: 0, bottom: 0, right: 0, zIndex: 0 },
  overlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(10, 26, 26, 0.7)', zIndex: 1 },
  mainWrapper: { flex: 1, zIndex: 2 },
  content: { flex: 1, padding: 20, justifyContent: 'center' },
  header: { alignItems: 'center', marginBottom: 25 },
  iconCircle: { width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(0, 43, 43, 0.6)', justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#00f2ff', marginBottom: 15 },
  title: { fontSize: 38, fontWeight: '900', color: '#fff', letterSpacing: 4 },
  subtitle: { fontSize: 13, color: '#00f2ff', letterSpacing: 2 },
  scanLine: { position: 'absolute', width: '100%', height: 2, backgroundColor: 'rgba(0, 242, 255, 0.4)', top: 40 },
  card: { backgroundColor: 'rgba(0, 26, 26, 0.85)', borderRadius: 20, borderWidth: 1, borderColor: '#00f2ff', overflow: 'hidden' },
  cardPadding: { padding: 25 },
  instruction: { fontSize: 14, color: '#fff', fontWeight: 'bold', marginBottom: 15, textAlign: 'center' },
  inputSection: { marginBottom: 30 },
  inputBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 12, height: 65, borderWidth: 3, borderColor: '#004d4d' },
  inputBoxFocused: { borderColor: '#00f2ff' },
  countryPickerTrigger: { flexDirection: 'row', alignItems: 'center', paddingLeft: 12, borderRightWidth: 1, borderRightColor: 'rgba(255,255,255,0.2)', height: '100%', paddingRight: 8 },
  prefixText: { color: '#fff', fontSize: 16, fontWeight: 'bold' },
  input: { flex: 1, padding: 15, fontSize: 22, color: '#fff', fontWeight: 'bold' },
  loadingBox: { alignItems: 'center' },
  loadingText: { color: '#00f2ff', fontSize: 12, marginVertical: 15, fontWeight: 'bold' },
  ...
  btn: { padding: 20, borderRadius: 12, alignItems: 'center', justifyContent: 'center', elevation: 8, shadowColor: '#00f2ff', shadowOpacity: 0.5, shadowRadius: 10 },
  btnActive: { backgroundColor: '#00f2ff' },
  btnDisabled: { backgroundColor: '#002b2b', borderWidth: 1, borderColor: '#004d4d' },
  btnText: { color: '#000', fontWeight: '900', fontSize: 20, letterSpacing: 2 },
  ...
  searchInput: { backgroundColor: '#f5f5f5', padding: 15, borderRadius: 12, marginBottom: 20, color: '#000' },
